...
/* The transport primitives on their own, so a bundle that is not webpack — and
 * therefore has no module to swap — can have them spliced in beside its
 * existing io-client instead. `indent` is the column the block is written at. */
function transportPrimitives(p, indent) {
  const pad = " ".repeat(indent);
  const body = PRIMITIVES_SOURCE
    .replace(/\$\{p\.signatureBytes\}/g, p.signatureBytes)
    .replace(/\$\{p\.encryptedMode\}/g, p.encryptedMode)
    .replace(/\$\{p\.tableSalt\}/g, p.tableSalt)
    .replace(/\$\{JSON\.stringify\(p\.c2sAlphabet\)\}/g, JSON.stringify(p.c2sAlphabet))
    .replace(/\$\{JSON\.stringify\(p\.s2cAlphabet\)\}/g, JSON.stringify(p.s2cAlphabet));
  if (indent === 16) return body;
  return body.split("\n").map((l) => (l.trim() ? pad + l.slice(16) : l)).join("\n");
}

/* Wraps those primitives in an IIFE that exposes only what a patch site needs,
 * keeping two names in the host scope rather than a dozen. */
function transportModule(p, name) {
  return `const ${name} = (function () {
${transportPrimitives(p, 4)}
    return {
        SIG_BYTES: SIG_BYTES,
