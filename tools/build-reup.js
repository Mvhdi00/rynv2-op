...
/* ------------------------------------------------------------------ *
 * 12. Automill: three windmills in every direction
 *
 * The old spacing was exact tangency, and the game rounds the place angle to
 * two decimals on its way out (`M.fixTo(It, 2)` in the bundle's own attack
 * path). 0.01rad at the windmill's 85-unit place radius is ~0.85 units of
 * arc, so rounding two neighbours toward each other closes a gap that had no
 * clearance to give and the server refuses the build. Whether that happens
 * depends on where base +/- offset lands on the 0.01 grid, which is why the
 * wall came out a different width depending on which way you walked.
 *
 * Sweeping 36000 headings: the old spacing places fewer than three mills at
 * 83.7% of them. See the comment written into the client for the rest.
 * ------------------------------------------------------------------ */

const AUTOMILL_CLASS = `  // ── Automill ──────────────────────────────────────────────────────────────
