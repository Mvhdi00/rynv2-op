 ...
 * build-player.js
 *
 * The player build is the owner build with a short list of changes, and nothing
 ...
 *   3. the menu rows for owner-only features, which read as dead switches in a
 *      player build because the code behind them is gated on RYN_IS_OWNER_BUILD
 *
 * Every edit is anchored to an exact string, and an anchor that is missing or
 * matches more than once fails the build rather than half-applying.
 *
...
const OWNER_ONLY_ROWS = [
  { page: "Visuals", id: "_markRynPlayers" },
  { page: "Misc", id: "_rynWelcome" },
];
...
function cutRow(html, id) {
  ...
  const open = html.lastIndexOf('<div class="content-option"', 0 + at);
  ...
  /* Take the trailing newline+indent with it so the page keeps its shape. */
  const tail = html.slice(end).match(/^\r?\n\s*/);
  const cut = html.slice(0, open) + html.slice(end + (tail ? tail[0].length : 0));
