...
MUTATIONS = [
  ...
  ("spike tick stops recognising an existing spike",
   "            if (this._alreadyCovered()) {", "            if (false) {"),
  ("spike tick acquire goes back to angles[0] only",
   "        if (!engine._conflicts.availableGround(c)) continue;", "        // dropped"),
