...
// @version      1.0
// @description  Install this once, then run old/broken moomoo mods unchanged. Restores the transport the server now requires and translates packet names between mod generations.
// @author       -
// @match        *://*.moomoo.io/*
// @grant        none
 ...
 *   - a msgpack codec on window.msgpack, byte-identical to the game's, so
 *     mods whose CDN died still have one
 *   - the Cloudflare Turnstile token, captured for mods that need it
 *
 * WHAT IT CANNOT HANDLE
 *
 *   - bugs specific to one mod: a wrong @run-at, an unguarded getElementById
 *     that throws on the first line, dead CDN @requires, a stale connect URL
 *   - mods that decode with their own bundled codec straight off the raw
 *     socket event rather than via window.msgpack. Hook-style mods (the common
 *     kind, where the real game client is still running underneath) are fine;
 *     full client replacements that own the socket outright are not.
 *   - anything the server changed in a packet's *payload*, as opposed to its
 *     name
 *
 * So: it fixes the protocol for you. It is not a guarantee that any given mod
 * will work.
 * ======================================================================== */
