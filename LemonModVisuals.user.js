 ...
 * READ THIS FIRST
 *
 * This script targets the 2019-generation moomoo protocol. Its packet
 * vocabulary is a full generation behind the live server:
 *
 *   outgoing here : sp ch 33 pp rmd 13c 2 5 7 8 10 11 12 14 ...
 *   outgoing live : M D 9 e F z H K L N b P Q c 6 S 0
 *
 *   incoming here : sp ch h ac ad an st sa us mm 33 1 2 4 ...
 *   incoming live : A B C D E a G H I J K L M N O P Q R S T U V X Y Z g 0-9
 *
 * 14 of the 17 names this script sends have no opcode on the current server,
 * and nine incoming names (1 2 4 5 6 7 8 9 a) exist in BOTH vocabularies
 * meaning different packets -- old "5" was a leaderboard update, current "5"
 * is a store update.
 *
 * The shim below makes the transport correct: the io-init handshake, the
 * shuffled opcode tables and the HMAC framing the server now requires. That is
 * necessary but NOT sufficient. Until the packet names above are mapped onto
 * the current ones -- by matching each handler's behaviour, not by guessing,
 * since the overlapping names would silently dispatch the wrong handler with
 * the wrong arguments -- this script will still not play.
 *
 * Unknown names are dropped by the shim rather than sent as garbage, so the
 * failure is quiet and clean instead of corrupting game state.
 * ------------------------------------------------------------------------ */
const EXP = (function() {
    "use strict";
		...
		"./src/js/libs/io-client.js": function(e, t, n) {
			...
			e.exports = {
				...
				connect: function(e, t, n) {
					if(!this.socket) {
						...
						try {
							var s = !1,
								o = e;
							this.socket = new WebSocket(o), this.socket.binaryType = "arraybuffer";
							var handshakeDone = !1;
							this.socket.onmessage = function(e) {
								// The shim records this socket's key/tables from
								// io-init and maps numeric opcodes back to names.
								var p = EXP.receive(this, e.data);
								if (!p) return;
								var o = p.type,
									t = p.args;
								if ("io-init" == o) {
									i.socketId = t[0];
									// Fire the connect callback here, not on open:
									// nothing may be sent before the tables exist.
									if (!handshakeDone) { handshakeDone = !0; t2(); }
									return
								}
								n[o] && n[o].apply(void 0, t)
				...
				send: function(e) {
					...
					// Framed with this socket's opcode tables + HMAC key. Names
					// the current server has no opcode for are dropped here --
					// which, for this script, is most of them. See the note at
					// the top of the file.
					EXP.send(this.socket, e, t)
				},
...
} // end __lemonVisualsBoot

if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", __lemonVisualsBoot, { once: true });
} else {
    __lemonVisualsBoot();
}
